  ...
  return (
    ...
    <>
      ...
      <div className='w-full h-24 flex flex-row shadow-lg border-b-1 border-black justify-left items-center bg-gradient-to-r from-rose-100 to-teal-100'> 
        <motion.div className='mx-8' animate={{y: move1? -10: 0}} onHoverStart={()=> {setMove1(!move1)}} onHoverEnd={()=> {setMove1(!move1)}}><h1 className='uppercase text-xl font-bold'><a href="/WebD-Workshop">techfest-workshop</a></h1></motion.div>
        ...
        <motion.div className='mx-8' animate={{y: move3? -10: 0}} onHoverStart={()=> {setMove3(!move3)}} onHoverEnd={()=> {setMove3(!move3)}}><h1 className='uppercase text-lg '><a href="/about">about</a></h1></motion.div>
        <div className=''></div>
